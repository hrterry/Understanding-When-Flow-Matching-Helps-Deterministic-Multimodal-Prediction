...
class HESTDataset(Dataset):
    def __init__(
        self,
        dataset: HESTDatasetPath,
        normalize_method,
        distribution="beta_3_1",
        sample_times=5,
        spot_sampling_mode: str = "local",
        max_sampled_spots: int | None = None,
    ):
        super().__init__()
        ...
        self.patch_h5_path = getattr(dataset, "patch_h5_path", None)
        ...
        self.patch_sampler = PatchSampler(
            distribution,
            mode=spot_sampling_mode,
            max_samples=max_sampled_spots,
        )
...
class MultiHESTDataset(Dataset):
    def __init__(
        self,
        dataset_list: List[HESTDatasetPath],
        normalize_method,
        distribution="beta_3_1",
        sample_times=5,
        spot_sampling_mode: str = "local",
        ...
        for i, dataset in enumerate(self.dataset_list):
            ...
            coords = data_dict["coords"]
            embeddings = data_dict["embeddings"]
            self.barcodes_list.append(barcodes)
            self.patch_h5_paths.append(getattr(dataset, "patch_h5_path", None))
